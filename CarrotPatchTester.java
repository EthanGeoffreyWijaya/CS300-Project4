...
public class CarrotPatchTester extends CarrotPatch {
  ...
  public static boolean test1isCloseMethod() {
    ...
    if (r.isClose(w1, Rabbit.getScanRange())) { // bug! isClose() should return false here
      System.out
          .println("Problem detected: Rabbit.isClose() returned true when it should return false.");
      return false;
    }
    if (!r.isClose(w2, Rabbit.getScanRange())) { // bug! isClose() should return true here
      System.out
          .println("Problem detected: Rabbit.isClose() returned false when it should return true.");
      return false;
    }
  ...
  public static boolean test2isCloseMethod() {
    ...
    if (!w.isClose(r1, Wolf.getScanRange())) {
      System.out
          .println("Problem detected: Wolf.isClose() returned false when it should return true");
      return false;
    }
    if (w.isClose(r2, Wolf.getScanRange())) {
      System.out
          .println("Problem detected: Wolf.isClose() returned ture when it should return false");
      return false;
    }
   ...
   * check whether this rabbit is close to a given (x,y) position located outside of its hopping
   * range
   * 
   * @return true when test verifies correct functionality, and false otherwise.
   */
  public static boolean test3isCloseMethod() {
    ...
    if (!r.isClose(200, 210, Rabbit.getHopStep())) {
      System.out
          .println("Problem detected: Rabbit.isClose() returned false when it should return true");
      return false;
    }
    if (r.isClose(300, 250, Rabbit.getHopStep())) {
      System.out
          .println("Problem detected: Rabbit.isClose() returned true when it should return false");
      return false;
    }
